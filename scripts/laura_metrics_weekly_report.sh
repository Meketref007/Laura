...
ENABLED="${LAURA_METRICS_WEEKLY_REPORT_ENABLED:-1}"
WINDOW_DAYS="${LAURA_METRICS_WEEKLY_REPORT_WINDOW_DAYS:-7}"
TREND_STABLE_DELTA="${LAURA_METRICS_WEEKLY_TREND_STABLE_DELTA:-2}"
...
if [[ "$ENABLED" == "0" ]]; then
  echo "Laura weekly metrics report disabled by LAURA_METRICS_WEEKLY_REPORT_ENABLED=0"
  exit 0
fi
...
if [[ -z "${LAURA_ALERT_TELEGRAM_BOT_TOKEN:-}" || -z "${LAURA_ALERT_TELEGRAM_CHAT_ID:-}" ]]; then
  echo "Telegram not configured; weekly summary: $summary_json"
  exit 0
fi
...
print(
    "Laura Weekly Metrics (${WINDOW_DAYS}d)\\n"
    f"Host: $(hostname)\\n"
...
entry = {
  ...
  "script": "laura_metrics_weekly_report.sh",
  "mode": "decision",
...
PY
          ...
          if bash -lc "$remediation_cmd" >/tmp/laura_auto_remediate_weekly.out 2>/tmp/laura_auto_remediate_weekly.err; then
            auto_line="Auto-Remediation: executed for ${top_cause} (success)"
            run_status="success"
          else
            auto_line="Auto-Remediation: executed for ${top_cause} (failed, check /tmp/laura_auto_remediate_weekly.err)"
            run_status="failed"
          fi
...
echo "Laura weekly metrics report sent."
